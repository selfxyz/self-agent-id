...
export default function CliQuickstartPage() {
  return (
    <main className="min-h-screen max-w-4xl mx-auto px-6 pt-24 pb-12">
      <h1 className="text-4xl font-bold text-center mb-8">CLI Quickstart</h1>

      <div className="space-y-6">
        <Card>
          <h2 className="text-lg font-bold mb-2">What this page is</h2>
          <p className="text-sm text-muted">
            This is the entrypoint for CLI onboarding. The CLI prints a signed
            handoff URL that opens{" "}
            <code className="bg-surface-2 px-1 rounded text-accent-2">
              /cli/register
            </code>{" "}
            for browser proof completion.
          </p>
        </Card>
        ...
        <Card>
          ...
          <ol className="list-decimal list-inside text-sm text-muted space-y-2">
            ...
            <li>
              Complete Self proof in browser (and passkey step for smart-wallet
              mode).
            </li>
            <li>
        ...
        <Card>
          ...
          <ol className="list-decimal list-inside text-sm text-muted space-y-2">
            ...
            <li>
              Get handoff URL:{" "}
              <code className="bg-surface-2 px-1 rounded text-accent-2">
                self-agent deregister open --session
            ...
            <li>
              Finalize in terminal:{" "}
              <code className="bg-surface-2 px-1 rounded text-accent-2">
                self-agent deregister wait --session
                .self/session-deregister.json
              </code>
            </li>
          </ol>
        ...
        <Card>
          ...
          <p className="text-sm text-muted mb-2">
            Your backend or agent runtime can orchestrate the same commands,
            then send the handoff URL to the user.
          </p>
          <ol className="list-decimal list-inside text-sm text-muted space-y-1">
            <li>
              Backend calls{" "}
              <code className="bg-surface-2 px-1 rounded text-accent-2">{`{register|deregister} init`}</code>{" "}
              and stores session state.
            </li>
            <li>
              Backend calls{" "}
              <code className="bg-surface-2 px-1 rounded text-accent-2">{`{register|deregister} open`}</code>{" "}
              and forwards URL to user UI.
            </li>
            <li>User completes browser proof flow.</li>
            <li>
              Backend runs{" "}
              <code className="bg-surface-2 px-1 rounded text-accent-2">{`{register|deregister} wait`}</code>{" "}
              and records the returned lifecycle state.
            </li>
          </ol>
        </Card>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-muted">
                ...
                to register, sign, and verify agents directly from your editor —
                no CLI needed.
              </p>
            </div>
          </div>
