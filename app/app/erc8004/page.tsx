    ...
    <main className="min-h-screen max-w-3xl mx-auto px-6 pt-24 pb-12 space-y-12">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-muted max-w-2xl mx-auto">
          An optional extension to ERC-8004 that adds on-chain,
          privacy-preserving proof-of-human verification for AI agents, proposed
          as a new section in the EIP, similar to how ERC-721 defines optional
          Metadata and Enumerable extensions.
        </p>
      </div>
      ...
      <section className="space-y-4">
        ...
        <p className="text-muted leading-relaxed">
          ERC-8004 is a proposed standard for on-chain AI agent registries. It
          defines a minimal interface for registering agents, assigning them
          unique IDs (as NFTs), and looking up agent ownership. Think of it as
          ENS for AI agents: a universal, composable identity layer.
        </p>
        <CodeBlock
          tabs={[
            {
...
interface IERC8004 {
    ...
    function ownerOf(uint256 agentId) external view returns (address);
        ...
        <p className="text-sm text-muted">
          The base standard is intentionally minimal. It doesn&apos;t specify{" "}
          <em>how</em> agents are verified or <em>who</em> operates them.
          That&apos;s where optional extensions come in.
        </p>
      </section>
      ...
      <section className="space-y-4">
        ...
        <Card>
          <p className="text-muted leading-relaxed">
            ERC-8004 registers agents, but it doesn&apos;t answer the critical
            question:
            <strong className="text-foreground">
              {" "}
              &ldquo;Is this agent operated by a real human?&rdquo;
            </strong>
          </p>
          <p className="text-muted leading-relaxed mt-3">
            Without proof-of-human, anyone can register unlimited agents,
            enabling sybil attacks, bot farms, and impersonation. Protocols that
            gate access to &ldquo;verified agents&rdquo; have no standard way to
            check humanity. Each project builds its own solution, fragmenting
            the ecosystem and creating integration overhead.
          </p>
        </Card>
      </section>
      ...
      <section className="space-y-4">
        ...
        <p className="text-muted leading-relaxed">
          We propose{" "}
          <strong className="text-foreground">IERC8004ProofOfHuman</strong>, an
          optional extension interface that adds proof-of-human verification as
          a first-class, composable property of agent identity. Registries that
          need human verification implement this alongside the base standard.
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card glow>
            ...
            <p className="text-sm text-muted">
              Any ZK identity system (Self Protocol, World ID, Humanity
          ...
          <Card glow>
            ...
            <p className="text-sm text-muted">
              Each human produces a unique, scoped nullifier. The registry
              tracks agent counts per nullifier. Services choose their own
              limits (1, N, or unlimited).
            </p>
          </Card>
          <Card glow>
            ...
            <p className="text-sm text-muted">
              ...
              no biometrics. ZK proofs verify humanity without revealing
              identity.
            </p>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-sm text-muted mb-3">
              Extends ERC-8004 with human verification, sybil detection, and
              provider management. Registries opt in by implementing this
              interface alongside the base standard.
            </p>
            <CodeBlock
              tabs={[
                {
...
interface IERC8004ProofOfHuman is IERC8004 {
    ...
    function isApprovedProvider(address provider) external view returns (bool);
          ...
          <div>
            ...
            <p className="text-sm text-muted mb-3">
              Pluggable interface for identity verification backends. Any ZK
              identity system can implement this to serve as a proof provider.
            </p>
            <CodeBlock
              tabs={[
                {
    ...
    function verificationStrength() external view returns (uint8);
      ...
      <section className="space-y-4">
        ...
        <p className="text-muted">
          ERC-8004 defines three registry types. Self Agent ID implements all
          three as on-chain contracts:
        </p>
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <p className="text-xs text-muted mt-1">
              Agent NFT minting, human proof storage, ZK-attested credentials,
              and metadata (A2A Agent Cards).
            </p>
          </Card>
          <Card>
            ...
            <p className="text-xs text-muted mt-1">
              Reads{" "}
              <code className="text-accent-2">verificationStrength()</code> from
              the proof provider. Supports batch queries across multiple agents.
            </p>
          </Card>
          <Card>
            ...
            <p className="text-xs text-muted mt-1">
              Real-time proof validation with configurable freshness threshold
              (default: ~1 year).
            </p>
          </Card>
        </div>
        ...
        <Card>
          ...
          <p className="text-xs text-muted mb-3">
            A2A Agent Cards are stored on-chain in{" "}
            <code className="text-accent-2">agentMetadata</code>. A stateless
            API reads the blockchain and serves the data as HTTP JSON. That URL
            becomes the agent&apos;s{" "}
            <code className="text-accent-2">agentURI</code> in ERC-8004.
          </p>
...
GET /.well-known/a2a/{agentId}?chain={chainId}  → Redirect to card resolver`}
          ...
          <p className="text-xs text-muted mt-3">
            For fully on-chain contexts, the SDK generates{" "}
      ...
      <section className="space-y-4">
        ...
        <Card variant="warn">
          <div className="flex items-start gap-3">
            <GitPullRequest
              size={24}
              className="text-accent-warn flex-shrink-0 mt-0.5"
            />
            <div>
              <p className="font-bold mb-1">Pull Request: Coming Soon</p>
              <p className="text-sm text-muted">
                We&apos;re preparing a PR to the ERC-8004 EIP document proposing
                proof-of-human as an optional extension section, similar to how
                ERC-721 defines optional Metadata and Enumerable extensions
